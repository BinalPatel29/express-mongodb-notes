...
router.get('/', asyncHandler(async (req: CustomRequest, res: Response): Promise<Response | void> => {
  ...
  const responsePayload = { success: true, data: notes, pagination: { totalItems, totalPages: Math.ceil(totalItems / parsedLimit), currentPage: parsedPage } };
  if (redis) {
...
router.post('/upload', asyncHandler(async (req: CustomRequest, res: Response): Promise<Response | void> => {
  ...
  const result = validateNote(payload); // ◄ Fixed to match discriminated union variant properties
  if (!result.success) {

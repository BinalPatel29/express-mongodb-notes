...
export interface IImageJobPayload {
  ...
  userId: string;
  filename: string;
  _telemetryContext?: Record<string, string>;
}

const redisOptions = process.env['REDIS_URL']
  ? { url: process.env['REDIS_URL'], maxRetriesPerRequest: null }
  : { host: process.env['REDIS_HOST'] || "127.0.0.1", port: parseInt(process.env['REDIS_PORT'] || "6379", 10), maxRetriesPerRequest: null };
...
let workerInstance: Worker<IImageJobPayload> | null = null;

export function startImageWorker(): Worker<IImageJobPayload> {
  if (workerInstance) return workerInstance;

  workerInstance = new Worker<IImageJobPayload>("imageJobQueue", async (job: Job<IImageJobPayload>) => {
    const tracer = trace.getTracer('worker-process');
    const parentContext = propagation.extract(context.active(), job.data._telemetryContext || {});

    return context.with(parentContext, async () => {
      const span = tracer.startSpan('Worker.optimizeImage');
      ...
      try {
        ...
        span.recordException(workerError instanceof Error ? workerError : new Error(String(workerError)));
        span.setStatus({ code: 2, message: workerError?.message || String(workerError) });
        endTimer();
        queueActiveWorkers.dec();
        throw workerError;
      } finally {
